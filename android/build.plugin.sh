...
pushd ../../sdk-facebook/android > /dev/null
ant clean
ant release
popd > /dev/null
...
OUTPUT_PLUGIN_DIR_ANDROID=$path/build/android
rm -rf $OUTPUT_PLUGIN_DIR_ANDROID
mkdir -p $OUTPUT_PLUGIN_DIR_ANDROID

if [ "$OUTPUT_PLUGIN_DIR_ANDROID" ]
then
	cp -v "$path/metadata.lua" "$OUTPUT_PLUGIN_DIR_ANDROID"
	cp -v "$JAR_PATH" "$OUTPUT_PLUGIN_DIR_ANDROID"
	cp -v "$path/../../sdk-facebook/android/libs/"* "$OUTPUT_PLUGIN_DIR_ANDROID"
	cp -v "$path/../../sdk-facebook/android/bin/classes.jar" "$OUTPUT_PLUGIN_DIR_ANDROID/facebook_sdk.jar"
	mkdir -p "$OUTPUT_PLUGIN_DIR_ANDROID/resources/res"
	cp -r "$path/../../sdk-facebook/android/res/" "$OUTPUT_PLUGIN_DIR_ANDROID/resources/res"
